  ...
  return (
    <div className="group flex items-center gap-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-fs-warm-gray transition-all hover:shadow-md hover:ring-fs-copper/30">
      ...
      {item.external ? (
        <ExternalLink
          size={14}
          className="shrink-0 text-fs-copper-light opacity-0 transition-opacity group-hover:opacity-100"
        />
      ) : (
        <ArrowRight
          size={14}
      ...
      {items.length === 0 ? (
        <div className="rounded-2xl bg-white p-8 text-center shadow-sm ring-1 ring-fs-warm-gray">
          ...
          <p className="mt-3 text-sm">
            <Link
              href="/dashboard"
              className="font-semibold text-fs-copper hover:text-fs-espresso"
            >
              Browse the Toolbox
            </Link>
