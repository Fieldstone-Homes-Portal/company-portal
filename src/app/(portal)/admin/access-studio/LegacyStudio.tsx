...
export default function AccessStudio({
  ...
  const deptMembers = useMemo(() => {
    ...
    for (const p of people) for (const id of p.deptIds) m.get(id)?.push(p);
    return m;
  }, [departments, people]);
  ...
  async function saveApp(e: React.FormEvent) {
    ...
    setSavingApp(false);
    if (!res.ok)
      return info("Couldn't save the app — check the fields and try again.");
  ...
  function resolvedPeople(
    ...
    if (!g) return [];
    if (g.everyone) return people.map((p) => ({ person: p, via: "All staff" }));
  ...
  return (
    <div className="space-y-8">
                            ...
                            {g &&
                              ...
                              g.userIds.length === 0 && (
                                <p className="w-full rounded-xl border border-dashed border-fs-warm-gray px-3 py-3 text-center text-[11px] text-fs-copper-light">
                                  No grants yet. Use the Grant access tab.
                                </p>
                              )}
                          </>
                        )}
        ...
        <>
          ...
          <section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-fs-warm-gray">
            ...
            <div className="flex flex-wrap gap-2">
              {visiblePeople.map((p) => {
                const isSel =
                  selected?.kind === "person" && selected.id === p.id;
                return (
                  <span
                    key={p.id}
                    draggable
      ...
      {settingsOnly && (
        <section className="rounded-xl border border-fs-warm-gray bg-white p-5">
          <h2 className="mb-3 text-2xl">People settings</h2>
          <div className="flex flex-wrap gap-2">
            {people.map((p) => (
              <button
                key={p.id}
                className="rounded-md border border-fs-warm-gray px-3 py-2 text-sm"
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                <label className={labelClass}>
                  Tags{" "}
                  <span className="text-fs-copper-light">
                    — navigation only, never access. Manage tags under Admin →
                    Tags.
                  </span>
                </label>
                {tags.length === 0 ? (
              ...
              <div className="sm:col-span-2">
                <label className="flex items-center gap-2 text-sm text-fs-espresso">
                  ...
                  <span className="text-xs text-fs-copper-light">
                    — disabled apps stay configured but are hidden from everyone
                  </span>
                </label>
              </div>
            </div>
